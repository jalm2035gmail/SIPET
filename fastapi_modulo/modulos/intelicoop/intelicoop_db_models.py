...
class IntelicoopHistorialPago(Base):
...
class IntelicoopTransaccion(Base):
...
class IntelicoopProspecto(Base):
...
class IntelicoopContactoCampania(Base):
...
class IntelicoopSeguimientoCampania(Base):
...
class IntelicoopScoringResult(Base):
