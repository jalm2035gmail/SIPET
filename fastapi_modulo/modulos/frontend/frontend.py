...
_BUILDER_TEMPLATE = os.path.join("fastapi_modulo", "modulos", "frontend", "frontend.html")
...
def api_list_forms():
    ...
    try:
        ...
        try:
            forms = (
                db.query(_core.FormDefinition)
                .filter(_core.FormDefinition.is_active == True)  # noqa: E712
                .order_by(_core.FormDefinition.name)
                .all()
            )
            return {
                "success": True,
