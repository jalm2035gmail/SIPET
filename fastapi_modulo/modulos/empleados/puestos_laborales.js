(function() {
        ...
        var html = Object.keys(groups).sort(function(a, b) { return a.localeCompare(b, 'es'); }).map(function(name) {
            var cards = groups[name].map(function(p) {
                ...
                return '<article class="rounded-box border border-base-300 bg-base-100 p-3 grid gap-1">' +
                    ...
                    '<span class="text-sm text-base-content/70">' + _esc(p.nivel || 'Sin nivel') + '</span>' +
    ...
    function renderOrganigramaView() {
        ...
        if (!data.length) {
            host.innerHTML = '<p class="text-sm text-base-content/60">Sin puestos registrados.</p>';
            return;
        }
        ensurePlOrgLibrary().then(function(libOk) {
            if (!libOk) {
                host.innerHTML = '<p class="text-sm text-base-content/60">No se pudo cargar la librería de organigrama.</p>';
                return;
            }
            var areaNodesMap = {};
            var nodes = [];
