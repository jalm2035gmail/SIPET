...
def _build_reportes_content(can_access_all: bool) -> str:
    ...
    return (
        ...
        "<img src='/icon/reporte.svg' alt='Icono reportes' width='96' height='96' class='shrink-0 rounded-box border border-base-300 bg-base-100 p-3 object-contain' />"
        ...
        "<div class='block w-full text-base sm:text-lg text-base-content/70'>Analítica y exportación con filtros y permisos por rol.</div>"
        "</div></div></div>"
        ...
        f"<p class='text-sm italic text-base-content/70'>{escape(scope_text)}</p>"
        ...
        "</table></div>"
        "<p id='rep-msg' class='text-sm text-base-content/60'></p>"
        "</div></article></section>"
        "</section>"
        "<script>"
        "(function(){"
        ...
        "const renderRows=(rows)=>{if(!body)return;body.innerHTML=(rows||[]).map((r)=>'<tr><td>'+ (r.actividad||'N/D') +'</td><td>'+ (r.responsable||'N/D') +'</td><td>'+ (r.usuario||'N/D') +'</td><td>'+ (r.sucursal||'N/D') +'</td><td>'+ (r.departamento||'N/D') +'</td><td>'+ (r.region||'N/D') +'</td><td>'+ (r.eje||'N/D') +'</td><td>'+ (r.objetivo||'N/D') +'</td></tr>').join('') || '<tr><td colspan=\"8\" class=\"text-center text-base-content/50\">Sin resultados</td></tr>';};"
        "const renderKpis=(summary)=>{if(!kpis)return;const cards=[['Actividades',summary.total_actividades],['Usuarios',summary.total_usuarios],['Departamentos',summary.total_departamentos],['Regiones',summary.total_regiones]];kpis.innerHTML=cards.map((c)=>'<article class=\"stat rounded-box border border-base-300 bg-base-100\"><div class=\"stat-title text-xs\">'+c[0]+'</div><div class=\"stat-value text-2xl\">'+String(c[1]||0)+'</div></article>').join('');};"
