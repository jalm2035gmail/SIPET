...
ssh "$SERVER" "touch '${REMOTE_DIR}.env' && \
  ...
  grep -q '^DATABASE_URL=' '${REMOTE_DIR}.env' && sed -i 's|^DATABASE_URL=.*|DATABASE_URL=sqlite:////var/lib/avancoop/data/strategic_planning.db|' '${REMOTE_DIR}.env' || echo 'DATABASE_URL=sqlite:////var/lib/avancoop/data/strategic_planning.db' >> '${REMOTE_DIR}.env' && \
  grep -q '^SQLITE_DB_PATH=' '${REMOTE_DIR}.env' && sed -i 's|^SQLITE_DB_PATH=.*|SQLITE_DB_PATH=/var/lib/avancoop/data/strategic_planning.db|' '${REMOTE_DIR}.env' || echo 'SQLITE_DB_PATH=/var/lib/avancoop/data/strategic_planning.db' >> '${REMOTE_DIR}.env' && \
